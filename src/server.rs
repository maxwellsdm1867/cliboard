...
fn handle_post_chat(mut request: tiny_http::Request, session_dir: &Path, ws_clients: &WsClients) {
    ...
    match serde_json::from_str::<ChatRequest>(&body) {
        Ok(chat_req) => {
            ...
            // Fire custom reply hook if set (for non-Claude-Code users).
            // The default reply mechanism is the board agent sub-agent — no
            // inline claude -p fallback. Use CLIBOARD_REPLY_HOOK env var for
            // custom integrations.
            if let Ok(hook_cmd) = std::env::var("CLIBOARD_REPLY_HOOK") {
